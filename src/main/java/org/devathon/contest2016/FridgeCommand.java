		...
		if (cmd.getName().equalsIgnoreCase("fridge")) {
			if (args.length == 0) {
				String [] fridgeMsg = {
						...
						"by pushing the button, the fridge will grant you great gifts that can fill you empty stomach, so that you can live yet another day in this horrible world.",
				};
				...
				if (sender instanceof Player) {
					...
					DevathonPlugin.getInstance().getServer().broadcastMessage(player.getName() + " made a great sacrifice today");
					return true;
				} else {
